...
export function useThemeLogic() {
  ...
  const getIconClassName = (iconTheme: string) => {
    if (!isMounted)
      return 'absolute h-[1.2rem] w-[1.2rem] transition-all rotate-90 scale-0';
    const baseClasses = 'absolute h-[1.2rem] w-[1.2rem] transition-all';
    const activeClasses = 'rotate-0 scale-100';
    const inactiveClasses =
      'rotate-90 scale-0 fill-text/50 dark:fill-darkText/50';
    ...
    const fillClass =
      iconTheme === 'dark'
        ...
        : iconTheme === 'light'
          ? 'fill-main'
          : 'fill-main dark:fill-main/50';

    return `${baseClasses} ${
      mountedTheme === iconTheme
        ? `${activeClasses} ${fillClass}`
        : inactiveClasses
    }`;
  };
